...
int main() {
  ...
  for (ramAddress = 0; ramAddress < 512; ramAddress++) {
    asm volatile("l.nios_rrr r0,%[in1],r0,165" ::[in1] "r"(ramAddress | writeBit)); // we clear the memory
  }
  printf("Comparing\n");
  for (ramAddress = 0; ramAddress < 512; ramAddress++) {
  ...
  printf("Writing\n");
  for (ramAddress = 0; ramAddress < 512; ramAddress++) {
    asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(ramAddress | writeBit), [in2]"r"(ramAddress ^ 0xFFFFFF)); // we fill the memory
  }
  printf("Comparing\n");
  for (ramAddress = 0; ramAddress < 512; ramAddress++) {
  ...
  asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(busStartAddress | writeBit),[in2] "r"((uint32_t) &memBuffer[0]));
  asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(memoryStartAddress | writeBit),[in2] "r"(usedCiRamAddress));
  asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(blockSize | writeBit),[in2] "r"(usedBlocksize));
  asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(burstSize | writeBit),[in2] "r"(usedBurstSize));
  printf("Performing dma from memory to ciRam\n");
  asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(statusControl | writeBit),[in2] "r"(1));
  printf("verifying\n");
  for (ramAddress = usedCiRamAddress; ramAddress < usedCiRamAddress+usedBlocksize; ramAddress++) {
    asm volatile("l.nios_rrr %[out1],%[in1],r0,165" :[out1]"=r"(ramData):[in1] "r"(ramAddress&0x1FF)); // we control that the memory is correct
    if (ramData != 0) printf("Error at address 0x%03X : 0x%08X\n", ramAddress, ramData);
  }
  printf("Writing ci memory\n");
  for (ramAddress = 0; ramAddress < 512; ramAddress++) {
    asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(ramAddress | writeBit), [in2]"r"(ramAddress ^ 0xFFFFFF00)); // we fill the memory
  }
  printf("Performing dma from ciRam to memory\n");
  asm volatile("l.nios_rrr r0,%[in1],%[in2],165" ::[in1] "r"(statusControl | writeBit),[in2] "r"(2));
  printf("verifying\n");
