module ramDmaCi #( parameter [7:0] customId = 8'hA5 )
  ...
  wire s_isSramWrite = (valueA[31:11] == 22'd0) ? s_isMyCi & valueA[10] : 1'b0;
  ...
  always @*
    case (s_dmaCurrentStateReg)
      ...
      DO_WRITE              : s_dmaNextState <= (busErrorIn == 1'b1) ? END_TRANSACTION_ERROR :
                                                (s_wordsWrittenReg[9] == 1'b1 && busyIn == 1'b0) ? END_WRITE_TRANSACTION : DO_WRITE;
  ...
  wire s_doBusWrite = (s_dmaCurrentStateReg == DO_WRITE) ? ~busyIn & ~s_wordsWrittenReg[9] : 1'b0;
  ...
  always @(posedge clock)
    begin
      ...
      s_wordsWrittenReg   <= (s_dmaCurrentStateReg == SET_UP_TRANSACTION) ? {2'b0,s_usedBurstSize} : 
                             (s_doBusWrite == 1'b1) ? s_wordsWrittenReg - 10'd1 : s_wordsWrittenReg;
